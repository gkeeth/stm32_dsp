/*
 * Listing 2.9: Interrupt-based echo
 * in ----------------------------(+)--out
 *       |                         |
 *       --(+)--Delay > ------------
 *          |              |
...
#define BUFFER_SIZE 8000 // larger for slower echos
#define GAIN 0.8f // larger for slower decay
...
void spi2_isr(void) {
    ...
    if (I2S2_EXT_SR & SPI_SR_CHSIDE) {
        ...
        delayed = rbuffer[rbuf_index];
        right_out_sample = right_in_sample + delayed;
        rbuffer[rbuf_index] = right_in_sample + delayed * GAIN;
        ...
        delayed = lbuffer[lbuf_index];
        left_out_sample = left_in_sample + delayed;
        lbuffer[lbuf_index] = left_in_sample + delayed * GAIN;
